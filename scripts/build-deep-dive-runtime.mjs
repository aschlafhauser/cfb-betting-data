...
const appGames=await page.evaluate(async()=>{
  ...
  for(const x of all){
    ...
    try{
      if(typeof renderMatch==='function'&&x.id){renderMatch(x.id);await new Promise(r=>setTimeout(r,5));detail=(document.getElementById('matchDetail')?.innerText||'').slice(0,7000)}
    }catch{}
...
function evidenceFor(bg,ag){
  ...
  return {
    ...
    availability:{accounted:true,evidence:availabilityHit?'Material availability/depth-chart language is present in governed matchup detail.':'No material availability item surfaced in the governed matchup detail at capture time; continue late-week recheck.'},
    coachingScheme:{accounted:true,evidence:coachingHit?'Governed team/matchup profile contains coaching/scheme/situational context.':'No matchup-specific coaching change surfaced; baseline system/team identity remains the governed context.'},
    ...
    expertResidual:{accounted:true,evidence:expertHit?'Governed matchup detail contains current expert-source context; same-family repetition remains correlated.':'No new matchup-specific expert residual surfaced in the rendered governed detail; this is explicitly neutral, not positive evidence.'},
...
const dossiers=(board.games||[]).map(bg=>{
  ...
  if(prior&&['COMPLETE','COMPLETE-SOURCE-LIMITED'].includes(prior.status)&&Array.isArray(prior.mechanisms)&&prior.mechanisms.length>=2){
  ...
  return {
    canonicalGameId:bg.gameId,
    legacyGameId:null,
    matchup:`${bg.away} at ${bg.home}`,
    status:full?'COMPLETE':'COMPLETE-SOURCE-LIMITED',
    informationQuality:full?'standard':'source-limited',
    priority:bg.priority||ag?.priority||'low',
    market:{spread:bg.currentSpread??null,total:bg.currentTotal??null,moneyline:bg.currentMoneyline??null,source:bg.marketSource??null,asOf:board.updatedAt||null},
    model:{spread:bg.modelSpread??ag?.fair??null,total:bg.modelTotal??ag?.modelTotal??null,edgeMagnitude:bg.modelEdgeMagnitude??null,independentFootballFair:bg.independentFootballFair??null,independentFootballSide:bg.independentFootballSide??null,independentFootballEdge:bg.independentFootballEdge??null},
    mechanisms:mechanisms.slice(0,7),
    evidenceDimensions:ev,
    expertResidual:ev.expertResidual.evidence,
    invalidators:ev.invalidators.evidence,
    execution:`No official wager solely from dossier completion. Apply Bet Activation Gate v1.1 with current executable book/line/juice and preserve expert evidence as qualitative residual.`,
    evidenceCount:accounted,
    sourceLimitedReason:full?null:(!ag?'No matching rendered portal matchup object; dossier is limited to canonical market/model runtime.':'One or more matchup dimensions lack direct governed source depth; limitation is explicit rather than silently filled.'),
