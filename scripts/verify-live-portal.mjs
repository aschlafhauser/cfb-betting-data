...
for(let attempt=1;attempt<=8;attempt++){
 ...
 if(structural&&decisionLayers&&specialty){await browser.close();console.log(`LIVE PORTAL VERIFIED: rows=${expected}; markets=${diag.markets}; meaningful models=${diag.meaningful}; Deep Dives=${diag.deepDiveComplete}/${expected}; dossiers=${diag.dossierCount}; movers=${diag.moverCount}; Matchup Center=YES; Longshot ${diag.longshotRendered}/${diag.longshotRuntime}, selected detail 9 components, /25 score + narratives=YES; Underdog visible=${diag.underdogVisibleOk}; browserErrors=0.`);process.exit(0)}
