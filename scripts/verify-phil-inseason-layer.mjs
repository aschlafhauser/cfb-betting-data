...
if(target){for(const token of [target.away,target.home,'Average Game Grade','Pass D','Special Teams','Model treatment'])if(!out.text.toLowerCase().includes(String(token).toLowerCase()))failures.push(`Phil in-season panel missing ${token}`)}
